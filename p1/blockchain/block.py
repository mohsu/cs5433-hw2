...
class Block(ABC, persistent.Persistent):
    ...
    def calculate_merkle_root(self):
        ...
        if len(self.transactions) == 0:
            ...
            while level > 0:
                ...
                for i in range(1 << level):
                    treenodes[level][i] = sha256_2_string(
                        treenodes[level + 1][2*i] + treenodes[level + 1][2*i + 1])
    ...
    def is_valid(self):
        ...
        if self.is_genesis:
            if not self.height == 0 or self.parent_hash != "genesis":
        ...
        if not self.is_genesis:
            ...
            txs = {}
            inputs = set()
            blocks = chain.get_chain_ending_with(self.parent_hash)
            for tx in self.transactions:
                receiver = None
                ...
                if tx.hash in txs or nonempty_intersection(blocks, chain.blocks_containing_tx.get(tx.hash, [])):
                ...
                for input_ref in tx.input_refs:
                    ...
                    input_tx_hash, input_index = input_ref.split(":")
                    ...
                    if input_tx_hash in txs:
                        out = txs[input_tx_hash].outputs[input_index]
                    elif input_tx_hash in chain.all_transactions and len(chain.all_transactions[input_tx_hash].outputs) > input_index:
                        out = chain.all_transactions[input_tx_hash].outputs[input_index]
                    else:
                    ...
                    elif not out.receiver == receiver:
                    ...
                    if input_ref in inputs or nonempty_intersection(blocks, chain.blocks_spending_input.get(input_ref, [])):
                    ...
                    if not nonempty_intersection(blocks, chain.blocks_containing_tx.get(input_tx_hash, [])) and not input_tx_hash in txs:
                        return False, "Input transaction not found"
                    else:
                        inputs.add(input_ref)
                ...
                for o in tx.outputs:
                    ...
                    # On failure: return False, "User inconsistencies"
                    if not o.sender == receiver:
                        return False, "User inconsistencies"
                    total_output += o.amount
